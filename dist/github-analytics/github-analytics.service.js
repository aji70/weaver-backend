...
exports.GithubAnalyticsService = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const rest_1 = require("@octokit/rest");
const date_fns_1 = require("date-fns");
let GithubAnalyticsService = class GithubAnalyticsService {
    configService;
    octokit;
    ...
    async getContributorAnalytics(owner, repo) {
        try {
            ...
            const contributorStats = await Promise.all(contributors.data.map(async (contributor) => {
            ...
            throw new Error(`Failed to fetch contributor analytics: ${error.message}`);
        }
    }
    async getCommitActivity(owner, repo, days = 30) {
        try {
            const since = (0, date_fns_1.format)((0, date_fns_1.subDays)(new Date(), days), "yyyy-MM-dd'T'HH:mm:ss'Z'");
            ...
            const commitStats = commits.data.reduce((acc, commit) => {
                const date = (0, date_fns_1.format)(new Date(commit.commit.author.date), 'yyyy-MM-dd');
    ...
    async getPullRequestAnalytics(owner, repo, state = 'all') {
        try {
            ...
            throw new Error(`Failed to fetch pull request analytics: ${error.message}`);
        }
    }
            ...
            return {
                ...
                latestRelease: releases.data[0]
                    ? {
                        ...
                        downloads: releases.data[0].assets.reduce((acc, asset) => acc + asset.download_count, 0),
                    }
                    : null,
    ...
    async getSecurityAlerts(owner, repo) {
        try {
            const alerts = await this.octokit.securityAdvisories.listRepositoryAdvisories({
                owner,
                repo,
            });
            return {
...
exports.GithubAnalyticsService = GithubAnalyticsService;
exports.GithubAnalyticsService = GithubAnalyticsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [config_1.ConfigService])
], GithubAnalyticsService);
//# sourceMappingURL=github-analytics.service.js.map